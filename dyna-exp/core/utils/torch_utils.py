...
def set_optimizer(params, cfg):
    """
    if the optimizer's initialization arguments are given by cfg file, then use the argument in cfg file
    otherwise use default argument value
    :param cfg:
    :return: optimizer
    """
    opt_func = getattr(torch.optim, cfg.optimizer_name)
    dict_args = {}
    args = get_args(opt_func)
    for k in args:
        if hasattr(cfg, k):
            dict_args[k] = getattr(cfg, k)
    dict_args['params'] = params
    return opt_func(**dict_args)
...
def select_device(gpu_id):
    ...
    if gpu_id >= 0:
        ...
        ParamConfig.DEVICE = torch.device('cpu')
...
def tensor(x):
    ...
    x = torch.tensor(x, device=ParamConfig.DEVICE, dtype=torch.float32)
    return x


def range_tensor(end):
    return torch.arange(end).long().to(ParamConfig.DEVICE)
    ...
    def __init__(self, network=None, grads=None):
        if grads is not None:
            ...
            for param in network.parameters():
                self.grads.append(torch.zeros(param.data.size(), device=ParamConfig.DEVICE))
