...
LEGEND = dict(
    ...
    handlelength=1.5, handletextpad=0.5, loc='lower left')
...
def plot(task, ax, runs, methods, args):
  ...
  plt.legend(loc="lower left").set_zorder(100000)
...
def main(args):
  ...
  baselines = [Run("loca_phase_1", "optimal", None, None, np.array([4000]))] + \
              [Run("loca_phase_2", "optimal", None, None, np.array([2000]))] + \
              [Run("loca_phase_3", "optimal", None, None, np.array([2000]))]
